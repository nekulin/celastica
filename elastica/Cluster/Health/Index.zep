...
class Index
{
    ...
    {
        var shards, dataShards, shardNumber, shard;

        let shards = [];
        let dataShards = this->_data["shards"];
        for shardNumber, shard in dataShards {
            let shards[] = new \Elastica\Cluster\Health\Shard(shardNumber, shard);
        }
